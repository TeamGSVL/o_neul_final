...
let alcoholTextList =[,
    '소주의 효능'+'\n'+
    '1. 혈전의 예방효과'+'\n'+
    '혈전을 녹이기 위해서는 플라스민(Plasmin)이라는 혈액 내에 존재하는 산소의 작용이 필요하게 된다.'+'\n'+
    '알코올은 플라스민을 증가시키는 기능이 있는 것으로 알려져 있으며 그중에서도 소주는 특히 많은 플라스민을 증가시키는 능력이 있다는 실험 결과가 발표되었다.'+'\n'+
    '2. 미용 효과'+'\n'+
    '소주에는 폴리페놀의 일종인 항산화성분인 안토시아닌이 함유되어 있다.'+'\n'+
    '이것의 강한 항산화 작용에 의해 노화 방지, 혈액 정화, 눈의 건강을 유지하는 효과를 기대할 수 있다.'+'\n'+
    '지나친 음주는 건강을 해칩니다.\n'
    ,
    '맥주 효능'+'\n'+
    '1. 신장 건강 강화'+'\n'+
    ...
    '또한 맥주에 들어있는 홉 때문에 적당히 마시는 것은 신장 결석이 생기는 위험을 40% 까지 줄여준다.'+'\n'+
    '2. 소화 개선'+'\n'+
    ...
    '술은 나쁜 콜레스테롤을 줄이고 고혈압에 기여하는 효소의 생성을 자연스럽게 억제함으로써 도움을 준다.'+'\n'+
    '2. 변비 예방'+'\n'+
    '연구에 따르면 한국 전통 막걸리는 프로바이오틱한 잠재력을 가지고 있다.'+'\n'+
    '프로바이오틱스는 장을 건강하게 유지하고 감염으로부터 보호하는 것으로 알려져 있다.'+'\n'+
    '막걸리는 요구르트에서 발견되는 수준보다 높은 수준의 젖산 및 유산균을 함유하며 식이 섬유가 포함되어 있어 변비 예방에 도움이 된다.'+'\n'+
    '지나친 음주는 건강을 해칩니다.\n'
    ,
    '한국에는 술을 단순하게 다섯가지 종류로 나누는 경향이 있는데, 이것이 맥주, 소주, 청주, 막걸리, 그리고 양주이다.'+'\n'+
    '술의 제조법이나 제조국과 무관하게 단순히 외국술이라는 이유로 부르는 것이기 때문에 엄밀한 분류법은 아니다.'+'\n'+
    '양주는 보통 위스키를 가리키는 말로 쓰이지만,'+'\n'+
    '색이 갈색이고 맛이 쓰면 대충 양주로 퉁치는 분위기 때문에 위스키 외에도 브랜디, 럼, 데킬라 같은 술도 한 묶음으로 엮이며,'+'\n'+
    '심지어 리큐르나 색이 무색 투명한 보드카나 진도 종종 여기에 들어간다.'+'\n'+
    '와인이나, 일본의 사케 같은 경우는 외국 술이긴 하지만 다른 술과는 확연한 차이를 보이기에 구분해서 부른다.'+'\n'+
    '지나친 음주는 건강을 해칩니다.\n'
]
...
alcoholTitleElems.forEach(item=>{
    ...
    item.addEventListener('mouseover', e => {
        item.classList.add('alcohol_tile_click');
    })
...
let alcoholList = document.querySelector('.alcohol_num').value;
getFoodList((data) => {
    alcoholText(alcoholList);
    data.forEach(item=>{
        getImg(item, makeAlcoholList, 1);
